...
;(function($, window, document, undefined) {
	...
	jQuery.widget("moogle.contextmenu", {
		version: "1.2.2",
		options: {
			...
			// Events:
			beforeOpen: jQuery.noop,   // menu about to open; return `false` to prevent opening
			blur: jQuery.noop,         // menu option lost focus
			close: jQuery.noop,        // menu was closed
			create: jQuery.noop,       // menu was initialized
			createMenu: jQuery.noop,   // menu was initialized (original UI Menu)
			focus: jQuery.noop,        // menu option got focus
			open: jQuery.noop,         // menu was opened
			select: jQuery.noop        // menu option was selected; return `false` to prevent closing
		},
		/** Constructor */
		_create: function () {
			...
			if(opts.preventSelect){
				...
				targetId = (jQuery(this.element).is(document) ? jQuery("body") : this.element).uniqueId().attr("id");
				this.$headStyle = jQuery("<style class='moogle-contextmenu-style'>")
					.prop("type", "text/css")
			...
			this.element.delegate(opts.delegate, eventNames, jQuery.proxy(this._openMenu, this));
		},
		/** Destructor, called on jQuery().contextmenu("destroy"). */
		_destroy: function(){
		...
		_createUiMenu: function(menuDef){
			...
			if( ! menuDef ){
				return;
			} else if(jQuery.isArray(menuDef)){
				this.$menu = jQuery.moogle.contextmenu.createMenuMarkup(menuDef);
				this.menuIsTemp = true;
			}else if ( typeof menuDef === "string" ){
				this.$menu = jQuery(menuDef);
			}else{
				this.$menu = menuDef;
			}
...
//				.addClass("moogle-contextmenu")
				...
				.menu({
					blur: jQuery.proxy(this.options.blur, this),
					create: jQuery.proxy(this.options.createMenu, this),
					focus: jQuery.proxy(this.options.focus, this),
					select: jQuery.proxy(function(event, ui){
						...
						ui.target = jQuery(this.currentTarget);
		...
		_openMenu: function(event){
			var opts = this.options,
				...
				self = this,
				ui = {menu: this.$menu, target: jQuery(event.target)};
			...
			jQuery(document).bind("keydown" + this.eventNamespace, function(event){
			...
			if (jQuery.isFunction(posOption)) {
				posOption = posOption(event, ui);
			}
			posOption = jQuery.extend({
				my: "left top",
				at: "left bottom",
		...
		/** Handle jQuery().contextmenu("option", key, value) calls. */
...
//            return this.$menu && this.$menu.is(":visible");
		...
		setEntry: function(cmd, titleOrData){
			...
			if(typeof titleOrData === "string"){
				...
				jQuery.moogle.contextmenu.createEntryMarkup(titleOrData, $parent);
			}
		},
 ...
 * Global functions
 */
jQuery.extend(jQuery.moogle.contextmenu, {
	...
	createEntryMarkup: function(entry, $parentLi){
		...
		if( !/[^\-\u2014\u2013\s]/.test( entry.title ) ){
			...
			if( jQuery.isFunction(entry.action) ){
				$a.data("actionHandler", entry.action);
			}
			if(entry.uiIcon){
				$a.append(jQuery("<span class='ui-icon'>").addClass(entry.uiIcon));
			}
			if(entry.disabled){
				$parentLi.addClass("ui-state-disabled");
			}
			if(jQuery.isPlainObject(entry.data)){
				$a.data(entry.data);
			}
		}
		return $a;
	...
	createMenuMarkup: function(options, $parentUl){
		var i, menu, $ul, $li;
		if( $parentUl == null ){
			$parentUl = jQuery("<ul class='ui-helper-hidden'>").appendTo("body");
		}
		for(i = 0; i < options.length; i++){
			menu = options[i];
			$li = jQuery("<li>").appendTo($parentUl);

			jQuery.moogle.contextmenu.createEntryMarkup(menu, $li);

			if( jQuery.isArray(menu.children) ){
				$ul = jQuery("<ul>").appendTo($li);
				jQuery.moogle.contextmenu.createMenuMarkup(menu.children, $ul);
			}
		}
		return $parentUl;
	}

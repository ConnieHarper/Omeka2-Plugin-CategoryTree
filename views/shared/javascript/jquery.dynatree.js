...
/* Check browser version, since jQuery.browser was removed in jQuery 1.9 */
...
function getDtNodeFromElement(el) {
	...
	return jQuery.ui.dynatree.getNode(el);
/*
	var iMax = 5;
	while( el && iMax-- ) {
...
//		this.tree.logDebug ("%s.remove()", this);
	...
	removeChild: function(tn) {
		...
		if(this.ul){
//			jQuery("li", jQuery(this.ul)).remove(); // issue 399
...
//					this.ul.removeChild(tn.li);
					jQuery("li", jQuery(this.ul)).remove(); // issue 231
				}
//				delete tn;  JSLint complained
			}
...
//				self.tree.logDebug("appendAjax().success");
				...
				// http://code.google.com/p/dynatree/issues/detail?id=202
				   ...
				   data = (typeof data.d) == "string" ? jQuery.parseJSON(data.d) : data.d;
				}
...
//		}
		...
		case "helper":
			...
			var $helper = jQuery("<div class='dynatree-drag-helper'><span class='dynatree-drag-helper-img' /></div>")
				.append(jQuery(event.target).closest(".dynatree-title").clone());
//			    .append(jQuery(event.target).closest('a').clone());
			// issue 244: helper should be child of scrollParent
			jQuery("ul.dynatree-container", node.tree.divTree).append($helper);
//			jQuery(node.tree.divTree).append($helper);
...
//			this.logDebug("helper.sourceNode=%o", $helper.data("dtSourceNode"));
		...
		case "enter":
			...
			if(!res){
				...
				res = {
					over: ((res === true) || (res === "over") || jQuery.inArray("over", res) >= 0),
					before: ((res === true) || (res === "before") || jQuery.inArray("before", res) >= 0),
					after: ((res === true) || (res === "after") || jQuery.inArray("after", res) >= 0)
				};
			}
...
//		if( parseFloat(jQuery.ui.version) < 1.8 ) {
		if(versionCompare(jQuery.ui.version, "1.8") < 0){
...
*/
	...
	bind: function() {
		...
		this.element.bind(eventNames, function(event){
			var dtnode = jQuery.ui.dynatree.getNode(event.target);
			if( !dtnode ){
 ...
 */
jQuery.ui.dynatree.version = "$Version: 1.2.4$";
...
//		parent = $el.parents("[dtnode]").first(),
	...
	$el.parents().each(function(){
		node = useProp ? jQuery(this).prop("dtnode") : jQuery(this).attr("dtnode");
		if(node){
			return false;
		}
	});
	...
	if(dnd && dnd.onDragStart ) {
		tree.$tree.draggable({
			...
			helper: function(event) {
				var sourceNode = jQuery.ui.dynatree.getNode(event.target);
...
var _registerDnd = function() {
	...
	jQuery.ui.plugin.add("draggable", "connectToDynatree", {
		start: function(event, ui) {
			// issue 386
			var draggable = jQuery(this).data("ui-draggable") || jQuery(this).data("draggable"),
...
//				logMsg("    draggable.offset.click FIXED: %s/%s", draggable.offset.click.left, draggable.offset.click.top);
		...
		drag: function(event, ui) {
			...
			var draggable = jQuery(this).data("ui-draggable") || jQuery(this).data("draggable"),
				...
				targetNode = jQuery.ui.dynatree.getNode(event.target);
//			logMsg("jQuery.ui.dynatree.getNode(%o): %s", event.target, targetNode);
